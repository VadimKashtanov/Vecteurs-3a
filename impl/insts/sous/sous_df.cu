	...
	float sng_soustraction)
{
	...
	if (_y < Y && _t < GRAND_T) {
		...
		atomicAdd(&dx0[tx0*X0 + _y], sng_soustraction * dy[ty*Y + _y]);
	}
};

__global__
static void d_kerd__sub2(
	uint x0_t, uint X0, float * x0, float * dx0,
...
void sous__df(Inst_t * inst, float ** x__d, float ** dx__d, uint * ts__d, uint mega_t) {
	...
	//
	if (xs_existants == 2) {
		d_kerd__sub2<<<dim3(KERD(Y,16), KERD(GRAND_T,16)), dim3(16,16)>>>(
		...
		//
		d_kerd__sub1<<<dim3(KERD(Y,16), KERD(GRAND_T,16)), dim3(16,16)>>>(
